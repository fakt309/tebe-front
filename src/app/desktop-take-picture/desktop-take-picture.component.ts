import { Component, OnInit, Input, Output, EventEmitter, ElementRef, SimpleChanges, HostListener, OnDestroy } from '@angular/core'
import { AsyncService } from '../async.service'
import { Touch2Service, Touch } from '../services/touch2.service'

@Component({
  selector: 'app-desktop-take-picture',
  templateUrl: './desktop-take-picture.component.html',
  styleUrls: ['./desktop-take-picture.component.scss']
})
export class DesktopTakePictureComponent implements OnInit, OnDestroy {

  @Input() value: string = ''
  @Input() sublimeTakePicture: boolean = false
  @Input() sublimeSave: boolean = false
  @Input() sublimeCancel: boolean = false
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['sublimeTakePicture'] && changes['sublimeTakePicture'].previousValue !== changes['sublimeTakePicture'].currentValue && changes['sublimeTakePicture'].previousValue != undefined) {
      const vid = this.host.nativeElement.querySelector(".preview")
      const photo = this.host.nativeElement.querySelector(".photo")
      this.takePhoto()
      const canvas = document.createElement('canvas')
    ...
    if (changes['sublimeCancel'] && changes['sublimeCancel'].previousValue !== changes['sublimeCancel'].currentValue && changes['sublimeCancel'].previousValue != undefined) {
      this.opacityPhoto = '0'
      this.opacityVideo = '1'
      this.save.emit('')
    }
